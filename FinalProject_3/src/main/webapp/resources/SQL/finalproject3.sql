show user;

select *
from tab;

commit;

rollback;

drop table tbl_student purge; 

alter table tbl_person add(identity  number(8)  not null);

select *
from tbl_person;

select *
from tbl_student;

delete from tbl_student where stuno='1';
delete from tbl_person where name='홍길동';


-- 댓글 tbl_comment
create table tbl_comment
(comseq     number               not null   -- 댓글번호(시퀀스사용)
,content    varchar2(1000)       not null   -- 댓글내용
,regDate    date default sysdate not null   -- 작성일자
,fk_seq     number               not null   -- 원게시물 글번호
,fk_stuno   number             not null     -- 학번
,status     number(1) default 1  not null   -- 글삭제여부
                                            -- 1 : 사용가능한 글,  0 : 삭제된 글
                                            -- 댓글은 원글이 삭제되면 자동적으로 삭제되어야 한다.
,constraint PK_tbl_comment_comseq primary key(comseq)
,constraint FK_tbl_comment_fk_stuno foreign key(fk_stuno) references tbl_student(stuno)
,constraint FK_tbl_comment_fk_seq foreign key(fk_seq) references tbl_board(seq) on delete cascade
,constraint CK_tbl_comment_status check( status in(1,0) ) 
);

create sequence commentSeq
start with 1
increment by 1
nomaxvalue
nominvalue
nocycle
nocache;


-- 일반게시판 tbl_board
create table tbl_board
(seq         number                not null       -- 글번호(시퀀스사용)
,subject     Nvarchar2(200)        not null       -- 글제목
,content     Nvarchar2(2000)       not null       -- 글내용   -- clob (최대 4GB까지 허용) 
,files        varchar2(100)                        -- 파일
,categoryno  number(8)             not null       -- 카테고리번호(1:자유,2:중고,3:모집)
,readCount   number default 0      not null       -- 글조회수
,regDate     date default sysdate  not null       -- 글쓴시간
,good        number(8) default 0                  -- 좋아요
,fk_stuno    number                not null       -- 학생번호
,name        varchar2(20)          not null       -- 글쓴이
,status      number(1) default 1   not null       -- 글삭제여부   1:사용가능한 글,  0:삭제된글
,commentCount  number default 0    not null       -- 댓글의 개수
,constraint PK_tbl_board_seq primary key(seq)
,constraint CK_tbl_board_status check( status in(0,1) )
,constraint FK_tbl_board_fk_stuno foreign key(fk_stuno) references tbl_student(stuno)
);

SELECT to_char(sysdate, 'yyyy/mm/dd')
FROM DUAL;

create sequence boardSeq
start with 1
increment by 1
nomaxvalue
nominvalue
nocycle
nocache;

-- 학생 tbl_student
create table tbl_student
(stuno     number   not null  -- 학생번호 (시퀀스사용)
,pwd        varchar2(200)  not null -- 비밀번호
,fk_perno     number   not null  -- 사람번호(학번,교수번호)
,constraint PK_tbl_student_stuno primary key(stuno)
,constraint FK_tbl_student_fk_perno foreign key(fk_perno) references tbl_person(perno)
);


create sequence stuSeq
start with 1
increment by 1
nomaxvalue
nominvalue
nocycle
nocache;


--- 사람 tbl_person (교수, 관리자, 학생 모두 포함)
create table tbl_person
(perno              number         not null        -- 사람번호(학번,교수번호)
,name               varchar2(30)   not null  -- 이름
,gender             varchar2(1)              -- 성별   남자:1  / 여자:2
,birthday           varchar2(10)             -- 생년월일 
,email              varchar2(200)  not null  -- 이메일 (AES-256 암호화/복호화 대상)
,address            varchar2(200)            -- 주소
,mobile             varchar2(200)            -- 휴대폰번호 (AES-256 암호화/복호화 대상) 
,identity           number(8)      not null  -- 0:학생, 1:교수, 2:관리자,직원
,fk_colno           number(8)      not null  -- 단과대학ID
,constraint PK_tbl_person_perno primary key(perno)
,constraint UQ_tbl_person_email  unique(email)
,constraint CK_tbl_person_gender check( gender in('1','2') )
,constraint FK_tbl_person_fk_colno foreign key(fk_colno) references tbl_college(colno)
);




-- 단과대학 tbl_college
create table tbl_college
(colno      number              not null        -- 단과대학ID (시퀀스사용 100부터 시작해서 1씩증가>>학과번호와 구분하기 위해)
,colname    Nvarchar2(2000)     not null        -- 단과대학명  
,constraint PK_tbl_college_colno primary key(colno)
);


create sequence colSeq
start with 100
increment by 1
nomaxvalue
nominvalue
nocycle
nocache;


-- 학과 tbl_department
create table tbl_major
(depseq      number                not null       -- 학과ID (시퀀스사용)
,fk_colno    number(10)        not null       -- 단과대학ID
,content     Nvarchar2(200)       not null       -- 학과명
,constraint PK_tbl_major_depseq primary key(depseq)
,constraint FK_tbl_major_fk_colno foreign key(fk_colno) references tbl_college(colno)
);


create sequence depSeq
start with 1
increment by 1
nomaxvalue
nominvalue
nocycle
nocache;







